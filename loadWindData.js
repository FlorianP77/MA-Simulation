var DataProcess = (function () {
    ...
    var findNearestCoordinate = function (array, value) {
        return array.reduce((nearestValue, currentValue) => {
            return Math.abs(currentValue - value) < Math.abs(nearestValue - value) ? currentValue : nearestValue;
        });
    }

    var getWindSpeed = function (lon, lat, lev) {
        var lonCoordinate = findNearestCoordinate(data.lon.array, lon);
        var latCoordinate = findNearestCoordinate(data.lat.array, lat);
        var levCoordinate = findNearestCoordinate(data.lev.array, lev);


        var windIndex = (2 * lonCoordinate) + (720 * 2 * (latCoordinate + 90));

        var u = data.U.array[windIndex];
        var v = data.V.array[windIndex];
